...
class StockGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Stock entity")
        ...
        .repeat(2) {
            ...
            .get("/jhipsterorder/api/stocks")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stock")
            .post("/jhipsterorder/api/stocks")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "stockCode":"SAMPLE_TEXT", "stockType":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stock_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stock")
                .get("/jhipsterorder${new_stock_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stock")
            .delete("/jhipsterorder${new_stock_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
